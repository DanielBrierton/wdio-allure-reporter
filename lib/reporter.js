...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('allure:addLabel', (data) => {
            if (this.currentTestCid) {
                const allure = this.getAllure(this.currentTestCid)
                const currentTest = allure.getCurrentTest()

                if (!currentTest || !data.label || !data.value) {
                    return
                }

                currentTest.addLabel(data.label, data.value)
            }
        })
    }
